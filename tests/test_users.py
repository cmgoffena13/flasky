...
class TestUserClass:
    ...
    def test_follow_posts(self):
        ...
        susan_post = Post(body="post from susan", author=susan, timestamp=now + timedelta(seconds=4))
        mary_post = Post(body="post from mary", author=mary, timestamp=now + timedelta(seconds=3))
        david_post = Post(body="post from david", author=david, timestamp=now + timedelta(seconds=2))
