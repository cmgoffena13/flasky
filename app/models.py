...
class User(PaginatedAPIMixin, UserMixin, db.Model):
    ...
    )  # this doesn't generate in alembic correctly..
    ...
    def is_following(self, user):
        return (
            self.followed.filter(followers.c.followed_id == user.user_id).count() > 0
        )  # Could be == 1...
    ...
    def followed_posts(self):
        ...
        ).filter(followers.c.follower_id == self.user_id)
    ...
    def new_messages(self):
        ...
        return (
            Message.query.filter_by(recipient_id=self.user_id)
    ...
    def add_notification(self, name, data):
        ...
        n = Notification(
            name=name, payload_json=json.dumps(data), users=self
    ...
    def launch_task(self, name, description, *args, **kwargs):
        rq_job = current_app.task_queue.enqueue(
            "app.tasks." + name, self.user_id, *args, **kwargs
        )
        task = Task(
            task_id=rq_job.get_id(), name=name, description=description, users=self
        )  # user=self; user = User instance
        db.session.add(task)
        return task
    ...
    def get_task_in_progress(self, name):
        return Task.query.filter_by(
            name=name, user_id=self.user_id, complete=False
        ).first()
    ...
    def revoke_token(self):
        self.token_expiration = datetime.now().replace(tzinfo=timezone.utc) - timedelta(
            seconds=1
        )
    ...
    def check_token(token):
        ...
        if user is None or user.token_expiration < datetime.now().replace(
            tzinfo=timezone.utc
        ):
            return None
        return user
...
class Post(SearchableMixin, db.Model):
    ...
    __primarykey__ = (
        "post_id"  # Used for ElasticSearch, does not support composite keys
    )
