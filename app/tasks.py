...
def _set_task_progress(progress):
    ...
    if job:
        ...
        task.users.add_notification(name='task_progress', data={'task_id': job.get_id(), 'progress': progress})
...
def export_posts(user_id):
    try:
        ...
        for index, post in enumerate(user.posts.order_by(Post.timestamp.asc()), start=0):
            data.append({'body': post.body, 'timestamp': post.timestamp.isoformat() + 'Z'})
            time.sleep(5)
            index += 1
            _set_task_progress(100 * index // total_posts)

        send_email('[Flasky] Your blog posts', 
